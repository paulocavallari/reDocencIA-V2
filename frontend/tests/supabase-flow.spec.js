...
const PLAN_TITLE = "Plano E2E Supabase";
const PLAN_PROMPT = "Gerar um plano objetivo com atividade prática, foco em inclusão e avaliação formativa.";
const TEST_IDENTIFIER = process.env.TEST_IDENTIFIER || "";
const TEST_PASSWORD = process.env.TEST_PASSWORD || "";
const HAS_SUPABASE_CREDENTIALS = Boolean(TEST_IDENTIFIER && TEST_PASSWORD);
...
test.describe.serial("supabase lesson-plan flow", () => {
  test.skip(!HAS_SUPABASE_CREDENTIALS, "TEST_IDENTIFIER and TEST_PASSWORD are required for Supabase-mode E2E.");
  ...
  test("supabase login succeeds", async () => {
    await page.goto("/", { waitUntil: "networkidle" });
    await expect(page).toHaveURL(/\/$/);
    await expect(page.getByRole("link", { name: /novo plano|criar novo plano/i }).first()).toBeVisible();
  });

  test("admin page shows Supabase Postgres badge", async () => {
    test.skip(!currentUser?.is_admin, "Supabase test user is not an admin.");

    await page.goto("/admin", { waitUntil: "networkidle" });
    await expect(page.getByRole("heading", { name: /ambiente, integrações e currículo/i })).toBeVisible();

    const dbCard = page.locator(".stat-card", {
      has: page.getByText(/banco de dados/i),
    });
    const dbBadge = dbCard.locator(".db-badge.db-badge--postgres");

    await expect(dbCard).toBeVisible();
    await expect(dbBadge).toContainText(/supabase postgres/i);
    await expect(dbCard.getByText(/conexão ok/i)).toBeVisible();
  });
  ...
  test("save and list the generated plan", async () => {
  ...
  test("cleanup the generated plan", async () => {
    expect(createdPlanId).toBeTruthy();
